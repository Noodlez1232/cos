 ...
 *"isrs.c". This is because all the IRQ controllers need to have a special 'end of interrupt'
 *or EOI signal(0x20) thrown at them to tell them that you've finished handling the interrupt.
 *This is given to the master controller (at 0x20) in all cases and when an IRQ from 8 to 15
...
//As of 8/3/17 this has been changed so that the handler must call its own EOI signal
