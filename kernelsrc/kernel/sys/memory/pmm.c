...
uint32_t mmap_base_addresses[128];
//This holds all the lengths of the memory map.
uint32_t mmap_lengths[128];
...
void pmm_init(multiboot_info_t* mbt)
{
	...
	while (mmap < (memory_map_t*)(mbt->mmap_addr + mmap_length))
	{
		//Now we get the base addresses of the memory map
		mmap_base_addresses[i] = (uint32_t)(mmap->base_addr_low & 0xFFFF) | (uint32_t)(mmap->base_addr_high & (0xFFFF << 16));
		...
		mmap = (memory_map_t*) ((uint32_t)mmap + mmap_size + sizeof(mmap_size));
		i++;
	}
